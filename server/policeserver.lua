...
AddEventHandler('ox_inventory:openInventory', function(type, targetId)
    ...
    -- Vérifie que le joueur source est police et en service
        ...
        embeds = {
            {
                ...
                color = 3447003, -- bleu
                footer = { text = "Système RDV Police - Click&Build" },
...
-- server.lua
RegisterNetEvent('Policejob:PriseEtFinservice')
AddEventHandler('Policejob:PriseEtFinservice', function(PriseOuFin)
    ...
    local name = xPlayer.getName() or ("[%d]"):format(src)

    -- 🔊 Notifie uniquement les AUTRES policiers (évite le doublon chez l'auteur)
    for _, playerId in ipairs(ESX.GetPlayers()) do
        if playerId ~= src then
            local targetPlayer = ESX.GetPlayerFromId(playerId)
            if targetPlayer and targetPlayer.job and targetPlayer.job.name == 'police' then
                TriggerClientEvent("esx:showNotification", playerId, ("📻 Radio • %s a %s."):format(name, PriseOuFin))
            end
        end
    end

    -- ✅ Laisse la notif de l'auteur au CLIENT (ex: "Vous avez pris/fini votre service")
    -- Si tu n'as PAS de notif côté client et que tu en veux UNE, décommente la ligne ci-dessous :
    -- TriggerClientEvent("esx:showNotification", src, ("Vous avez %s votre service."):format(PriseOuFin))
end)
...
-- ===== Don d’équipement (sécurisé : job + grade cap) =====
local function giveEquipment(playerId, desiredGrade)
    local xPlayer = ESX.GetPlayerFromId(playerId)
    if not xPlayer then return end

    local jobInfo  = xPlayer.getJob and xPlayer.getJob() or {}
    local jobName  = jobInfo.name or ''
    local maxGrade = tonumber(jobInfo.grade) or 0

    -- Sécurité: job requis
    if jobName ~= (ConfigPolice.JobRequired or 'police') then
        notifyESX(playerId, "~r~Accès refusé.")
        return
    end

    -- Grade demandé ≤ grade réel (et ≥ 0)
    local g = tonumber(desiredGrade)
    if not g then g = maxGrade end
    if g > maxGrade then g = maxGrade end
    if g < 0 then g = 0 end

    local equipment = ConfigPolice.EquipmentByGrade[g]
    if not equipment or not equipment.items then
        notifyESX(playerId, "Équipement introuvable pour ce grade.")
        return
    end
    ...
    for _, it in ipairs(equipment.items) do
        ...
        local metadata = it.metadata
            or (name:find('ammo', 1, true) and ConfigPolice.BuildAmmoMetadata and ConfigPolice.BuildAmmoMetadata(playerId))
            or (ConfigPolice.BuildWeaponMetadata and ConfigPolice.BuildWeaponMetadata(playerId))
            or nil
    ...
    notifyESX(playerId, ("Vous avez reçu l’équipement de %s."):format(equipment.label or ("grade "..g)))
end

RegisterNetEvent('armorypolice:giveEquipment', function(desiredGrade)
    giveEquipment(source, desiredGrade)
end)
...
AddEventHandler(ConfigPolice.RemoveItemEvent, function()
    ...
    -- ✅ Une seule notif (le client ne notifie plus)
    notifyESX(src, "Équipement rendu.")
end)

-- Armurerie : donner des munitions pour l’arme choisie (inchangé)
...
-- SV/API : appliquer une amende et créditer la société police
RegisterNetEvent('police:SendFacture', function(target, price)
    local src = source
    local officer = ESX.GetPlayerFromId(src)

    -- ✅ Sécurité : seul le job police peut facturer
    ...
    TriggerEvent('esx_addonaccount:getSharedAccount', 'society_police', function(account)
        if not account then
            TriggerClientEvent("esx:showNotification", src, "Compte société introuvable (society_police).")
            return
        end
...
AddEventHandler("tan:create", function(nameSuspect, ageSuspect, heightSuspect, nationalitySuspect, sexSuspect)
	local __src = source 
	local casierData = {}
	MySQL.Async.fetchAll("SELECT * FROM criminal_records_policec WHERE name = @name", {
		["name"] = nameSuspect
	}, function(casierData)
	...
	MySQL.Async.execute("INSERT INTO criminal_records_policec (depositary, name, age, height, nationality, sex, date) VALUES (@a, @b, @c, @d, @e, @f, @g)", {
...
ESX.RegisterServerCallback("tan:getList", function(source, cb)
	totalCasierData = {}
	MySQL.Async.fetchAll("SELECT * FROM criminal_records_policec", {}, function(data)
...
AddEventHandler("tan:deletecasier", function(name)
	local __src = source
	MySQL.Async.execute("DELETE FROM criminal_records_policec WHERE name = @a", {
		["a"] = name
	})
...
AddEventHandler("tan:addmotif", function(name, motif)
	local __src = source
	MySQL.Async.execute("INSERT INTO criminal_records_policec (depositary,name,motif,date) VALUES (@a,@b,@c,@d)", {
		["a"] = GetPlayerName(__src),
		["b"] = name,
		["c"] = motif,
		["d"] = GetActualTime()
...
ESX.RegisterServerCallback("tan:getCasier", function(source, cb)
	motifData = {}
	MySQL.Async.fetchAll("SELECT * FROM criminal_records_policec WHERE name = @a", {["@a"] = casiera}, function(data)
		for _, v in pairs(data) do
...
AddEventHandler("edit:motif", function(name, lastMotif ,newMotif)
	local __src = source
	MySQL.Async.execute("UPDATE criminal_records_policec SET motif = @a WHERE name = @b and motif = @c", {
		["a"] = newMotif,
		["b"] = name,
		["c"] = lastMotif
	})
...
AddEventHandler("delete:motif", function(name, motif)
	local __src = source
	MySQL.Async.execute("DELETE FROM criminal_records_policec WHERE name = @name and motif = @motif", {
		["name"] = name, 
		["motif"] = motif
	})
...
AddEventHandler("delete:allmotif", function(name)
	MySQL.Async.execute("DELETE FROM criminal_records_policec WHERE name = @name", {
		["name"] = name
	})
end)