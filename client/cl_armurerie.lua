...
zUI.SetItems(armoryMenu, function()
    -- Déposer toutes les armes (➜ 1 seule notif serveur)
    zUI.Button("Déposer ses armes", nil, {}, function(onSelected)
        if not onSelected then return end
        local ped = PlayerPedId()
        -- Retire TOUTES les armes du ped (local)
        RemoveAllPedWeapons(ped, true)
        -- ❌ plus de ESX.ShowNotification ici
        -- ➜ laisse le serveur notifier une seule fois :
    ...
    for _, e in ipairs(entries) do
        ...
        if grade >= gkey then
            ...
            zUI.Button(("Équipement de %s"):format(infos.label or ("grade " .. gkey)), nil, {}, function(onSelected)
                if not onSelected then return end
                -- ➜ On envoie le grade demandé (Recrue/Officier/…)
                TriggerServerEvent('armorypolice:giveEquipment', gkey)
            end)
        end
    end
